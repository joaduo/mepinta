...
class PostInitStrategyBase(object):
    ...
    def _initChildren(self, args, kwargs):
        ...
        if len(args) > 0:
            raise TypeError(
                'You should provide all "__post_init__" arguments as keywords.')
        ...
        for class_, func in funcs:
            ...
            try:
                ...
                if error_file == self.__cleanPycExt(__file__):
                    ...
                    # Create proper message to report the __post_init__
                    # location.
                    new_message = self.__reportLocalError(
                        tb_list, class_, func, error)
                else:
                    # The problem is not calling func.
                    # Format the message so that the location is not lost in
                    # the report.
                    new_message = self.__reportOutsideError(
                        tb_list, class_, error)
    ...
    def __getInitLists(self, class_, posts, already_added):
        ...
        if issubclass(class_, PostInitStrategyBase):
            ...
            # We end up in a superclass that is not this class. Consider it an
            # error
            raise TypeError(
                'InitStrategyBase should be on the main inheritance line.')
...
def smokeTestModule():
    ...
    cc = ConcreteClass(
        context=1, too=2, something=2312, value1=231, other_value=20, value=100, test=2)
