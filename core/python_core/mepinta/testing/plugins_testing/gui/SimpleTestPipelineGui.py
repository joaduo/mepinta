...
class SimpleTestPipelineGui(FrameworkBase):
    ...
    def __createWidget(self, node, prop):
        ...
        text_types = [
            'c.builtin.charp', 'cpp.std.string', 'python.builtin.str']
        ...
        number_types = ['c.builtin.int', 'c.builtin.double',
                        'python.builtin.int', 'python.builtin.float']
    ...
    def __createPanel(self):
        ...
        panel = Panel(
            "Properties", width=200, height=200, fixed=False, modal=False)
        widget_list = []
        ...
        for node in self.test_pline.getNodesDict().values():
            for prop in node.inputs.getProperties().values():
                ...
                if widget != None:
                    self.__widgetid_value[
                        self.__createWidgetId(node, prop)] = None
                    self.__widgetid_prop[
                        self.__createWidgetId(node, prop)] = prop
    ...
    def __setupNodebox(self, canvas_x, canvas_y):
        ...
        self.nodebox_pline = NodeBoxSimplePipelineOutput(
            self.test_pline.getPipeline(), canvas_x, canvas_y)
    ...
    def __registerPipelineUIFunctions(self, ui_runner):
        for prop in self.test_pline.uiDefaultEvaluated:
            ui_runner.appendDrawFunction(
                self.test_pline.evaluatePropForced, prop)
    ...
    def runWithInotify(self, inotify_mngr, canvas_x=700, canvas_y=600):
        ...
        # Create the events loop object that also supports events coming from
        # inotify
