...
class PluginsManagerBase(FrameworkBase):
    ...
    def __post_init__(self, data_types, processors, parent):
        ...
        self.dtype_pkg_mngr = PluginPackageManager(
            plugins_type='data_types', context=self.context)
        self.processor_pkg_mngr = PluginPackageManager(
            plugins_type='processors', context=self.context)
