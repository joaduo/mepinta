...
class PropertyManager(HiAutoBase):
    ...
    def createProperties(self, pline, prop_type, names, data_types, prop_flags=0):
        ...
        if prop_type == 'input':
            prop_ids = self.wrapped.createProperties(
                pline, names, dtype_ids, INPUT_PROPERTY_FLAG | prop_flags)
        elif prop_type == 'internal':
            prop_ids = self.wrapped.createProperties(
                pline, names, dtype_ids, INTERNAL_PROPERTY_FLAG | prop_flags)
        elif prop_type == 'output':
            prop_ids = self.wrapped.createProperties(
                pline, names, dtype_ids, OUTPUT_PROPERTY_FLAG | prop_flags)
        else:
            raise RuntimeError('unsupported property type: %r' % type)
        return prop_ids

    def _createFuncPropertiesSafe(self, pline, names, func_ptr_ids):
        filterFunc = lambda function: unwrapLo(
            function) not in self.context.context_lo.functions
        return {filterFunc: func_ptr_ids}

    @unwrap_decorator
    def createFuncProperties(self, pline, names, func_ptr_ids):
        prop_ids = self.wrapped.createFuncProperties(
            pline, names, func_ptr_ids)
        return prop_ids

    def _createFunctumPropertiesSafe(self, pline, names, func_ptr_ids, data_types, prop_flags=0):
        filter_functions = lambda function: unwrapLo(
            function) not in self.context.context_lo.functions
        filter_dtype = lambda dtype: unwrapLo(
            dtype) not in self.context.context_lo.data_types
    ...
    def createFunctumProperties(self, pline, names, func_ptr_ids, dtype_ids, prop_flags=0):
        prop_ids = self.wrapped.createFunctumProperties(
            pline, names, func_ptr_ids, dtype_ids, prop_flags)
        return prop_ids
