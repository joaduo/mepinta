...
class ProcessorProxy(FrameworkBase):
    ...
    def __post_init__(self, name):
        ...
        # TODO: make filters for these list, so that they get the correct
        # values
        self.non_cached_capable = []  # a string
        self.marked_outputs = []  # a function property
        # self.marked_inputs = [] #a function property #TODO: is this ever
        # used?
    ...
    def getRequiredDataTypes(self, types_classes=(DataPropertyProxy,)):
        ...
        # TODO def getRequiredDataTypes(self, *types_classes): if
        # len(types_classes) == 0: types_classes=[DataPropertyProxy]?
        ...
        for container in self.containers.values():
            ...
            for node_property in props.values():
                ...
                if node_property.data_type_version not in data_types[dt_name]:
                    # create a sorted list, so latest version is at end
                    insort_left(data_types[dt_name], node_property.data_type_version)
        return data_types
    ...
    def getFunctionsDict(self):
        ...
        for container in self.containers.values():
            ...
            container_funcs = container.getProperties(
                FunctionPropertyProxy, FunctumPropertyProxy)
            if set(container_funcs.keys()) & set(func_dict.keys()):
                ...
                msg += 'Plugin: %r Functions: %r ' % (
                    self.name, set(container_funcs.key()) & set(func_dict.keys()))
                self.log.e(msg)
    ...
    def setFunctionsId(self, func_dict_ids):
        ...
        for func_name, func_id in func_dict_ids.items():
            getattr(
                self.getContainer(func_name), func_name).func_ptr_id = func_id
