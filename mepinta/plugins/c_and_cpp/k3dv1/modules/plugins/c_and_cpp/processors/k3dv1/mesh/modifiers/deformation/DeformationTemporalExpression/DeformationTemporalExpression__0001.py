...
class DeformationTemporalExpression(PluginManifestBase):
  ...
  def define(self, inputs, internals, functions, outputs):
    ...
    inputs.direction = DataProperty('uint') #Forward, Backward, Bidirectional (need to review this)
    inputs.samples_amount = DataProperty('uint') #If bidirectional samples are doubled
    ...
    internals.samples_storage = DataProperty('internal_any') # mesh:frame_number
    #Output
