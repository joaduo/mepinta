...
class ValueManager(object):
  ...
  def set_std_strings(self,pline, prop_ids, prop_values):
    for index,p_id in enumerate(prop_ids):
      self.set_prop_value(pline, p_id, str_string_to_voidp(prop_values[index]))  
  def set_charps(self,pline, prop_ids, prop_values):
    for index,p_id in enumerate(prop_ids):
      self.set_prop_value(pline, p_id, str_charp_to_voidp(prop_values[index]))
  def set_doubles(self,pline, prop_ids, prop_values): #TODO: check if shedskin does any difference
    for index,p_id in enumerate(prop_ids):
      self.set_prop_value(pline, p_id, double_to_voidp(prop_values[index]))
  def set_ints(self,pline, prop_ids, prop_values):
    for index,p_id in enumerate(prop_ids):
      self.set_prop_value(pline, p_id, int_to_voidp(prop_values[index]))
...
def shedskin_ValueManager(context_lo,pline, prop_id):
  pvs = ValueManager(context_lo)
  prop_ids = [prop_id]
  
  pvs.markChangedProps(pline, prop_ids)
  pvs.markChangeSetChangedProps(pline, prop_ids)
  pvs.set_std_strings(pline, prop_ids, ['value'])
  pvs.set_charps(pline, prop_ids, ['value'])
  pvs.set_ints(pline, prop_ids,  [1])
  pvs.set_doubles(pline, prop_ids,  [1.1])
  
  pvs.get_std_strings(pline, prop_ids)
  pvs.get_charps(pline, prop_ids)
  pvs.get_doubles(pline, prop_ids)
  pvs.get_ints(pline, prop_ids)
