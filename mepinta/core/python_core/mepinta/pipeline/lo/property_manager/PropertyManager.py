...
class PropertyManager(object):
  ...
  def createFuncProperties(self, pline, prop_names, func_ptr_ids):
    ...
    for index, prop_name in enumerate(prop_names):
      ...
      self.p_value_mngr.replacePropValue(prop, anyPtrToVoidp(FunctionPropertyValue(func_ptr_ids[index])))
      prop_ids.append(pline.addProperty(prop))
    return prop_ids
  def createFunctumProperties(self, pline, prop_names, func_ptr_ids, dtype_ids, prop_flags):
    ...
    for index, prop_name in enumerate(prop_names):
      ...
      self.p_value_mngr.replacePropValue(prop, anyPtrToVoidp(FunctumPropertyValue(func_ptr_ids[index], functum_prop)))
      prop_ids.append(pline.addProperty(prop))
    return prop_ids
  ...
  def addNonCached(self, pline, dent_ids, dency_ids):
    logDebug('Setting non cached connections %r <-> %r' % (dent_ids, dency_ids))
...
def shedskin_PropertyManager(context_lo, pline, prop_id):
  ...
  pm.createFunctumProperties(pline, prop_names, func_ptr_ids=[0], dtype_ids=[0], prop_flags=0)
  pm.createProperties(pline, prop_names, dtype_ids=[0], prop_flags=INPUT_PROPERTY_FLAG)
