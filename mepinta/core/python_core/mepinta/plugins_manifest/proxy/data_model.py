...
class PropertyProxyQualifier(FrameworkObject):
  ...
  def __getattr__(self,name):
    '''
      Intercept declaration_order from this class and inherited classes. 
  ...
  def __eq__(self, other):
    '''
      When comparing declaration_order, use the real property without qualifications.
    '''
...
class BaseInOutPropertyProxy(PropertyProxy):
  ...
  def getRealDataTypeName(self, data_type_alias):
    ...
    #              ##
    #              'MP_generic_enum':'int',
    #              'mepinta::generic_enum':'int',#'generic_enum', #MP_generic_enum
    #              'MP_functum':'functum', #MP_functum
    #              'mepinta::functum':'functum', #MP_functum
    #              'MP_internal_any':'internal_any', #MP_internal_any
    #              'mepinta::internal_any':'internal_any', #MP_internal_any
    #              #k3d
    #              'k3d::mesh':'k3dv1_Mesh',#'k3dv1.mesh',
    #              #'k3d::filesystem::path':'k3dv1_FilesystemPath',#'k3dv1.filesystem.path',
    #              'k3d::filesystem::path':'charp',#'k3dv1.filesystem.path',
    #              'k3d::imaterial':'k3dv1_Material', #k3dv1.imaterial
    #              #####==========Correct ones!
    #              'char*':'GC_charp',#'charp',
    #              'std::string*':'std_string',
    #              ##
    ...
    #              'mepinta::internal_any*':'internal_any', #MP_internal_any
    #              ##
    #              'k3d::int32_t':'int', #'c_builtin.int'
    #              'k3d::double_t':'double',
    #              'k3d::string_t':'std_string', #'std.string'
    #              'k3d::bool_t':'int',
    #              ##
    #              'k3d::mesh*':'k3dv1_Mesh',#'k3dv1.mesh',
    #              #'k3d::filesystem::path':'k3dv1_FilesystemPath',#'k3dv1.filesystem.path',
...
class PropertyProxyContainer(FrameworkObject):
  ...
  def __setattr__(self, name, value):
    ...
    '''
    if not name.startswith('_%s'%self.__class__.__name__) \
...
class ProcessorProxy(FrameworkObject):
  ...
  def __init__(self, name):
    ...
    self.name = name
    self.containers = {}
    self.inputs = PropertyProxyContainer()
    self.internals = PropertyProxyContainer()
    self.outputs = PropertyProxyContainer()
    self.functions = PropertyProxyContainer()
  ...
  def get_container(self, func_name):
    '''Get the container for a specific function name. Names cannot be repeated across containers.'''
    containers = []
    for container in self.containers.values():
