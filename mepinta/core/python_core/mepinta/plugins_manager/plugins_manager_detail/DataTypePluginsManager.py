...
class DataTypePluginsManager(PluginsManagerBase):
  ...
  def loadDataType(self, data_type, minor_version, reload_):
    ...
    data_type_name, package = self.dtype_pkg_mngr.getPackageAndName(data_type)
    ...
    build_modules = self.dtype_pkg_mngr.getRevisionModules(package)
    ...
    if minor_version > build_modules['versions'][-1] and not reload_:
      raise PluginImportError('Requested minor_version=%r for data type %r is newer than the latest minor_version available.' % (minor_version, data_type_name))
